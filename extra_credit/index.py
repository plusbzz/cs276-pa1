...
def gammaEncodeNumber(number):
  s = bin(number)
  s = s[3:]
  s = '1'*len(s)+'0'+s
  return s
      
def gammaEncodeNumbers(numbers):
  encodedNumbers = ""
  ...
  encodedGaps = []
  for start in xrange(0,sz,8):
    byte = encodedNumbers[start:(start+8)]
    byte = int(byte,2)
...
def writeGammaEncodedGaps(strLine,outputFile,postingDictionary):
...
while len(block_q) > 1:
  ...
  if len(block_q) > 0:
    ...
    mergeBlocks(b1,b2,comb,posting_dict,writeGammaEncodedGaps)
